...
6. 写入章节前必须保留原工作区快照；系统会在 write_file 写入 `chapters/` 前自动 Git commit，若失败则停止写入并说明原因
7. 使用 write_file 将章节写入 `chapters/chXX-章节名.md`
8. 不更改 `setting/outline.md`
9. 使用 write_file 更新 `setting/progress.md`（当前进度 + 本章摘要）
10. 使用 read_file + write_file 更新 `setting/characters.md`（角色状态变化）

## 写作要求

- 严格遵循大纲的章节走向和摘要
- 每写一章前必须先保存原工作区版本，不能在自动快照失败时继续覆盖章节正文
